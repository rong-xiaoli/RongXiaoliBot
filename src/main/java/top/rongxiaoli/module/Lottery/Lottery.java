...
public class Lottery extends Module {
    ...
    private final String HelpContent = "/lottery [amount]\n" +
            "或/l [amount]\n" +
            "抽奖\n" +
            "抽奖随机数公式在GitHub上的/src/main/java/top.rongxiaoli/module/Lottery/backend/LotteryPool.java。\n" +
            "参数：\n" +
            "amount：抽奖时消耗的货币数。";
    /**
    ...
    public void FriendMain(String[] arrCommand, long Friend, Contact SubjectContact) {
        ...
        if (message.length >= 2) {
            if (Objects.equals(message[1], "pool")) {
                SubjectContact.sendMessage("当前奖池内共有：" + lotteryPool.getPool() + "枚金币");
                return;
            }
        }
        if (message.length == 2) {
